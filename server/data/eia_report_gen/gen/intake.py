...
def gaps(data: dict, max_ask: int = 6) -> dict:
    ...
    # 2.5) 封面署名（建设单位/编制单位/编制日期）—— 2026-09-24 补。
    #      它们**不是必填**（缺了不能拦住生成，见 schema 里那段注释），但一定会出现在封面，
    #      所以在基本信息问完之后主动问一句。放优先级 5：只有前面的判定输入与表一填完了、
    #      还有名额时才问得到（不会挤掉"项目名称"这类关键项）。
    for k in ("建设单位", "编制单位", "编制日期"):
        f = ks.get(k)
        if f:
            add(k, f.note or "封面要写这一项", 5)
    # 3) 挑哪些来问：**不能全被判定类占满** ——
