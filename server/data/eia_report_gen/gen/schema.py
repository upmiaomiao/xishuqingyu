...
FIELDS: list[Field] = [
    ...
    # ---------------- 封面：署名与日期（2026-09-24 补） ----------------
    # 用户反馈：「我告诉他报告编制单位是中节能，但是他好像并不能帮我写入 doc 文档中」。
    # 查下来是三处**同时**断了，所以无论怎么说都到不了文档：
    #   ① 这三个字段**不在本表里** —— 抽取时给模型的清单来自 schema.FIELDS，模型被要求
    #      "输出的 key 必须与清单完全一致"，于是它压根不会输出它们；用户那句话既没进事实表，
    #      **也没有任何提示**（`丢弃` 是 0 条，因为模型根本没提这一项）——「说了没反应」的由来；
    #   ② `docx_writer` 封面把这三格**写成常量**（建设单位/编制单位 = 【需人工补充】、
    #      编制日期 = " 年 月"），就算前面填上了，封面也不会用；
    #   ③ 所以这里补字段、docx 那边改成读真实值，两处都改才算接通。
    # `required=False` 是刻意的：**缺了不能拦住生成**（strict=True 的样例路径会因为缺必填而整份失败），
    # 但 `intake.gaps()` 会在基本信息问完之后主动问一句（见那里的"封面署名"段）。
    Field("建设单位", "建设单位", "str", required=False, group="封面",
          note="封面盖章的建设方（和「项目名称」不是一回事）"),
    Field("编制单位", "编制单位", "str", required=False, group="封面",
          note="出具这份报告的单位（写全称，封面要盖章）"),
    Field("编制日期", "编制日期", "str", required=False, group="封面",
          note="留空则封面上留「年 月」待手填，工具不会替你写今天的日期"),
]
