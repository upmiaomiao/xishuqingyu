...
def t_docx():
    ...
    # C9-① 配套（2026-09-23）：真实世界的"没填"其实是**字段在、行里空着**
    # （`环保措施` 是必填项，接口会拦"整个字段缺失"）。原先按"字段非空"判断，
    # 这种填报既不写【需人工补充】、又渲染一张空表 —— 看起来像"故意没写措施"。
    d4 = copy.deepcopy(d)
    d4["环保措施"] = [{"要素": "废气", "措施内容": "", "排放去向": ""}]
    p4 = os.path.join(tmp, "d.docx")
    docx_writer.build(d4, decide.decide(d4, crit), p4)
    doc4 = Document(p4)
    txt4 = "\n".join(x.text for x in doc4.paragraphs) + "\n" + \
        "\n".join(c.text for t in doc4.tables for r in t.rows for c in r.cells)
    check("[注入] 环保措施只有空行 → 正文写【需人工补充】",
          "工具不编措施：请填报" in txt4)
    check("[注入] 环保措施只有空行 → 表五不出现空行条目",
          "选用低噪声设备" not in txt4)
    return tmp
