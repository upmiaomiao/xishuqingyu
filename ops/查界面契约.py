...
def main():
    ...
    #
    # 2026-09-22 修一条**一直在红的**假警报（改动前就红，备份里核过）：
    #   界面读 `x['已审核']`（报告下拉框）与 `r['汇总页']`（批注版 PDF 返回），
    #   两个键都是后端真产出的（audit_routes.py 的 /reports、audit_pdf.py 的汇总页），
    #   但这条检查 ① 取样里没有 /reports 的返回，② 只扫 /data/eia_audit 下的 .py ——
    #   而这两个产出点在**站点目录** xishu_pipeline/ 里。前提不成立，断言自然红。
    # 修法是**把前提补全**（不是把断言放宽）：加上 /reports 取样、把站点代码也纳入扫描。
    rp = get("/reports")
    payload = (json.dumps(res, ensure_ascii=False) + json.dumps(rv2, ensure_ascii=False) +
               json.dumps(ex, ensure_ascii=False) + json.dumps(rp, ensure_ascii=False))
    src = ""
    for root in ("/data/eia_audit", "/home/test/xishu_qingyu_serve/xishu_pipeline"):
