#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""20 条垃圾焚烧题线上实跑（v2：完整保存引用正文与元数据）。

相比 v1 多存的字段：
  · sources[].text / title / standard_id / doc_type / status / rerank_score / vector_score / chunk_index
  · reasoning（站点自己的推理轨迹）、model、usage、latency_s
这些是判"有据/编造"的前提 —— 只给文件名，裁判无法核实答案里的条款和限值。

用法：python 验20条_线上实跑v2.py [--only 1,2]
输出：_工作记录/实跑20条_输出v2.jsonl
"""
...
def main() -> int:
    ...
    print(f"实跑 {len(items)} 条（完整存引用正文）→ {BASE}/hybrid_search\n" + "=" * 104, flush=True)
    rows = []
    for it in items:
        q = it["题目"]
        t0 = time.time()
        ...
        q_std = stds(q)
        # 题目点名的标准号，是否出现在引用正文/标题/standard_id 里（这才是"引非所问"的正解）
        ...
        rr = [s.get("rerank_score") for s in srcs if isinstance(s.get("rerank_score"), (int, float))]
        titles = [s.get("title") or "" for s in srcs]
        rec = {
            "序号": it["序号"], "题型": it["题型"], "语言": it.get("语言", "中文"), "题目": q,
        ...
        print(f"{'✅' if not err else '❌'} [{it['序号']:>2}] {it['题型'][:12]:<12} {dt:>5.0f}s  "
              ...
              f"标准号命中 正文{len(hit_body)}/{len(q_std)} 标题{len(hit_meta)}/{len(q_std)}  {err}", flush=True)
