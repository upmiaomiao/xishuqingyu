#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""补 12 条判分（评分口径与 评20条v2.py 完全一致，便于横向比较）。

输入：_工作记录/实跑补12条_输出.jsonl
输出：_工作记录/实跑补12条_评分.jsonl（失败时原始输出留档 _评补12条_原始输出.jsonl）
"""
...
IN = WS / "_工作记录" / "实跑补12条_输出.jsonl"
OUT = WS / "_工作记录" / "实跑补12条_评分.jsonl"
RAW = WS / "_工作记录" / "_评补12条_原始输出.jsonl"
...
def main() -> int:
    ...
    print(f"评审 {len(rows)} 条补题（{MODEL}，带引用正文）\n" + "=" * 104, flush=True)
    ...
    for r in rows:
        ...
        rec = {"序号": r["序号"], "题型": r["题型"], "工艺域": r.get("工艺域"),
               "主补域": r.get("主补域"), "语言": r.get("语言"),
        ...
        out.append(rec)
        print(f"  [{rec['序号']:>2}] 补{str(rec.get('主补域'))[:6]:<8}{rec['题型'][:10]:<11} "
              f"正确{rec['correctness']} 覆盖{rec['coverage']} "
              f"可操作{rec['actionability']} 有据{rec['grounding']} 边界{rec['boundary']} "
              f"均分{rec['均分']} 编造{len(rec['fabrications'])} gap={rec['data_gap']} "
              f"| {rec['brief'][:40]}", flush=True)
