...
def clean_sql(sql: str) -> str:
    """Clean SQL: remove comments, normalize whitespace, keep only first statement."""
    if not sql:
        return ""

    # Split by -- comment lines that start a new statement description
    # e.g., "-- 废气主要排放口：SELECT ..."
    # We want to extract only the first SELECT statement

    # First, normalize line endings
    sql = sql.replace("\r\n", "\n").replace("\r", "\n")

    # Look for pattern: -- comment followed by SELECT
    # Split by lines starting with --
    lines = sql.split("\n")

    # Find the first line that starts with SELECT or WITH (after optional whitespace)
    start_idx = None
    ...
    # Collect lines from start_idx until we hit another -- comment that starts a new statement
    result_lines = []
    for i in range(start_idx, len(lines)):
        line = lines[i]
        stripped = line.strip()

        # Stop if we hit a -- comment that looks like a new statement description
        # e.g., "-- 废气一般排放口：" or "-- 查询..."
        if stripped.startswith("--") and i > start_idx:
            # Check if next non-empty line starts with SELECT
            for j in range(i + 1, len(lines)):
                ...
                # No more lines, continue
                pass

            # Check if this -- line is followed by a SELECT (indicating new statement)
            remaining = "\n".join(lines[i:])
            if re.search(r"--[^\n]*\n\s*(?:SELECT|WITH)", remaining, re.IGNORECASE):
                break
